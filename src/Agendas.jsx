...
export default function Agendas({ user, pacienteData, onAddAppointment, appointments }) {
    ...
    const handleSpecialtySelect = async (specialty) => {
        ...
        try {
            ...
            const res = await api.get(`/api/especialidades/${specialty.idEspecialidad}/doctores`);
            setDoctors(res.data);
            setStep(2);
        } catch (error) {
...
// Paso 3: Seleccionar fecha
    const handleDateSelect = (date) => {
        const fechaSeleccionada = date.toDate();
        const ahora = new Date();
        const limite48Horas = new Date(ahora);
        limite48Horas.setHours(ahora.getHours() + 48);

        // Verificar que la fecha seleccionada tenga al menos 48 horas de anticipación
        if (fechaSeleccionada < limite48Horas) {
            alert('Debes agendar con al menos 48 horas de anticipación. Por favor selecciona otra fecha.');
            return;
        }
    ...
    useEffect(() => {
        if (selectedDoctor && selectedDate) {
            const fetchAvailableHours = async () => {
                try {
                    const res = await api.get(`/api/doctores/${selectedDoctor.id}/horarios/disponibles`, {
            ...
            console.log('doctor id:', selectedDoctor?.id, 'fecha:', selectedDate?.format('YYYY-MM-DD'));
            fetchAvailableHours();
        }
...
// Paso 4: Seleccionar hora
    const handleTimeSelect = (time) => {
        ...
        // 🔹 CORREGIDO: Usar pacienteData para el nombre
        const nombreCompleto = pacienteData ?
            `${pacienteData.nombre || ''} ${pacienteData.apellidoP || ''} ${pacienteData.apellidoM || ''}`.trim() :
            user?.name || 'Paciente';
        ...
        const appointmentInfo = {
            ...
            doctor: `Dr. ${selectedDoctor.nombre} ${selectedDoctor.apellidoP} ${selectedDoctor.apellidoM}`,
            consultorio: selectedDoctor.consultorio,
            date: selectedDate.format('DD/MM/YYYY'),
            day: selectedDate.locale('es').format('dddd'),
            time: time,
            hospital: 'Hospital General IMSS',
            backendData: {
                idPaciente: user.id,
                idDoctor: selectedDoctor.id,
                fechaHoraCita: dayjs(`${selectedDate.format('YYYY-MM-DD')}T${time}`).format('YYYY-MM-DDTHH:mm:ss')
            }
        };
    ...
    const handleConfirmAppointment = async () => {
        ...
        try {
            ...
            const registroCita = {
                ...
                idDoctor: selectedDoctor.id,
                fechaHoraCita: dayjs(`${selectedDate.format('YYYY-MM-DD')}T${selectedTime}`).format('YYYY-MM-DDTHH:mm:ss')
            };
    ...
    const isDateAvailable = (date) => {
        const fechaJS = date.toDate();
        return diasDisponibles.some(fechaDisponible =>
            fechaDisponible.toDateString() === fechaJS.toDateString()
        );
    };
    ...
    return (
        <div className="agendas-container">
            <div className="agendas-card">
                ...
                {step === 2 && (
                    <div className="step-container">
                        <h3>Selecciona un doctor de {selectedSpecialty.nombre || selectedSpecialty.name}</h3>
                ...
                {step === 3 && (
                    <div className="step-container">
                        <h3>
                            Selecciona una fecha para tu cita con{' '}
                            {selectedDoctor.nombre} {selectedDoctor.apellidoP}
                        </h3>
                ...
                {step === 5 && appointmentData && (
                    <div className="step-container">
                        ...
                        <div className="appointment-summary">
                            ...
                            <div className="summary-item">
                                <strong>Fecha:</strong> {appointmentData.date} ({appointmentData.day})
                            </div>
                            <div className="summary-item">
